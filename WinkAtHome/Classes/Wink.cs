...
public class Wink
{
        ...
        try
        {
                        ...
                        if (ikeys != null)
                        {
                            ...
                            if (device.issensor)
                            {
                                ...
                                if (capabilities.Count > 0)
                                {
                                    ...
                                    {
                                        DeviceStatus status = device.status.SingleOrDefault(s => s.name == capability);
                                        if (status != null)
                                        {
            ...
            foreach (Device device in _devices)
            {
                using (SQLiteConnection connection = new SQLiteConnection("Data Source=" + dbPath + ";Version=3;"))
                {
                    ...
                    {
                        command.CommandText = "select * from Devices where DeviceID = @ID";
    ...
    private static List<Robot> winkGetRobots()
    {
        try
        {
            if (_robots == null)
            {
                string hideEmpty = SettingMgmt.getSetting("Hide-Empty-Robots");
                ...
                foreach (JObject data in json["data"])
                {
                    ...
                    if (data["causes"] != null)
                    {
                        ...
                        if (causes.HasValues)
                        {
                            ...
                            {
                                Device device = Devices.SingleOrDefault(d => d.id == data["causes"][0]["observed_object_id"].ToString());

                                string strDeviceIden = device != null ? device.name : "device ID " + data["causes"][0]["observed_object_id"].ToString();

                                robot.name += " (" + strDeviceIden + " " + data["causes"][0]["observed_field"].ToString() + " warning)";
                            }
